...
const QuizEditPage = () => {
  ...
  // Form state hooks (always at the top, before any return)
  ...
  if (error || !quiz) return <div className="flex justify-center items-center min-h-screen text-red-500">{error || 'Quiz not found.'}</div>;
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col items-center py-8">
      <div className="w-full max-w-3xl bg-white rounded-2xl shadow-xl flex flex-col min-h-[80vh]">
        <div className="flex items-center justify-between px-8 pt-8 pb-4 border-b rounded-t-2xl">
          <button className="text-timelingo-gold font-bold text-lg hover:underline" onClick={() => navigate('/quiz-builder')}>← Back to Library</button>
        ...
        <div className="sticky bottom-0 bg-white z-10 rounded-b-2xl border-t px-8 py-4 flex justify-end gap-2 shadow-inner">
          <button className="bg-gray-300 hover:bg-gray-400 text-gray-800 font-bold py-2 px-4 rounded-xl transition" onClick={() => navigate('/quiz-builder')}>Cancel</button>
          <button className="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-xl transition" onClick={handleSave} disabled={saving}>{saving ? 'Saving...' : 'Save'}</button>
        </div>
      </div>
    </div>
  );
